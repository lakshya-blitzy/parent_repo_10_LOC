...
- **HTTP field names are case-insensitive** (RFC 9110, §5.1), and implementations legitimately differ: the Java tier's built-in server emits `Content-type`, `Content-length` and `Cache-control` rather than `Content-Type`, `Content-Length` and `Cache-Control`. The rule is not a per-word convention but a whole-string one — **the first character is capitalized and every other character is lower-cased**, measured directly against the runtime's own header map, which stores `X-A-B-C` as `X-a-b-c` and `WWW-Authenticate` as `Www-authenticate` — and there is no supported way to override it. Conformance assertions MUST therefore compare header **names** case-insensitively. Header **values** are exact, including the space after the semicolon in `application/json; charset=utf-8`.
...
#### The scope of the JSON `404`/`405` guarantee, stated normatively

This is the one place in this document where a guarantee is bounded rather than universal, so it is stated plainly rather than left to be inferred from the table that follows.

**The guarantee of §2 and §5.1 — that any method other than `GET`/`HEAD` is answered `405` with `Allow: GET, HEAD`, and any path other than the resolved path is answered `404` with a small JSON error body, both carrying `Content-Type: application/json; charset=utf-8` and `Cache-Control: no-store` — is scoped to request targets in origin-form with a single leading slash**: precisely the targets §5.4.1 admits to step 3, plus every target it maps to nothing while still reaching application code. Within that scope the guarantee is universal and identical at all three tiers, and it covers every target a conforming HTTP client can construct, every row of §5.1.1, and every method.

**Outside that scope, only the fail-safe invariant at the end of this section applies.** A target that its runtime refuses below routing gets that runtime's own refusal, in that runtime's own shape, and the three tiers are documented to differ. This is a real non-uniformity, not a formality: at Level 3 it covers nine measured request lines, and for four of them it means the method-before-path rule of §5.4 does not hold.

The consequence for anyone writing a check is direct, and is the reason this statement exists at all:

- A conformance check MUST assert the JSON `404`/`405` contract **only** for in-scope targets. A check that asserts a JSON body for `//health` or `OPTIONS *` passes at two tiers and fails at the third, and the failure is the check's, not the tier's.
- For out-of-scope targets a check MUST assert the **fail-safe invariant instead** — never `2xx`, never `5xx`, never a `status` member, and the listener still serving afterwards. Those four properties hold at every tier and are what actually protect a consumer.
- **This scope MUST NOT be narrowed by an implementation as a convenience.** It is bounded by a measured runtime limit at one tier, and nothing else. An implementation that let an additional target escape its router — by trimming, by normalizing, or by delegating routing to a URL parser — would be violating §5.4.1, not exercising this clause.

Why the boundary is accepted rather than removed: it exists at Level 3 only, and removing it would mean replacing `com.sun.net.httpserver` with a hand-written HTTP/1.1 accept loop over a raw socket — the only mechanism that can see these targets, since the runtime's dispatcher decides before any handler or filter runs. **The plan of record mandates `com.sun.net.httpserver` from the `jdk.httpserver` module for this tier and asserts that module's presence inside the runtime image, so re-platforming is not available**, and it would be the wrong trade even if it were: it would move a documented, fail-safe, cosmetic difference in a handful of error bodies into an undocumented hand-written request parser, in the one component whose entire job is to be trustworthy. The decision is therefore to **scope the guarantee, enumerate the exceptions exhaustively, and assert the fail-safe** — which is what the rest of this section does.

**The Level 3 exception set, measured request line by request line.** These nine are the forms a caller is most likely to construct, and each is answered by the runtime with a short `text/html` body carrying no `Cache-Control`. They are a *sample of a rule*, not the whole of it — the rule that closes the set is stated immediately after the table, and the remaining members are the rows already in the matrix further down:

| Request line | Answer at Level 3 | Answer at Levels 1 and 2 | Measured cause |
| --- | --- | --- | --- |
| `GET //health` | runtime `404` HTML | application `404` JSON | `new URI("//health")` → `authority=health`, `path=""` |
| `GET //foo` | runtime `404` HTML | application `404` JSON | same: authority absorbs the segment, path empty |
| `GET *` | runtime `404` HTML | application `404` JSON | `new URI("*")` → `path="*"`, no leading `/` |
| `OPTIONS *` | runtime `404` HTML, **no `Allow`** | application `405` + `Allow` | as above; **method never evaluated** |
| `POST *` | runtime `404` HTML | application `405` + `Allow` | as above; **method never evaluated** |
| `POST //health` | runtime `404` HTML | application `405` + `Allow` | empty parsed path; **method never evaluated** |
| `PUT //health` | runtime `404` HTML | application `405` + `Allow` | empty parsed path; **method never evaluated** |
| `GET //` | runtime `400` HTML, body names `URISyntaxException` | application `404` JSON | `new URI("//")` → `URISyntaxException: Expected authority` |
| `CONNECT <host>:<port>` (authority-form) | runtime `400` HTML, body names `URISyntaxException` | `405` + `Allow` | `new URI("127.0.0.1:8080")` → `URISyntaxException: Illegal character in scheme name` |

**The rule that closes the set, which is what makes the exception bounded rather than open-ended.** At Level 3 a target reaches the router **if and only if `java.net.URI` reports a non-null path beginning with `/`** — because contexts are looked up by that parsed path and every legal context path starts with a slash. Everything else is refused below routing. Stating the rule rather than only the list matters, because the list is longer than the nine above; the rule's other members are all measured and all already recorded in the matrix below:

| Additional below-routing form | Parsed path | Answer at Level 3 |
| --- | --- | --- |
| Absolute-form with no path: `http://any.host`, `http://127.0.0.1:8080` | `""` (empty) | runtime `404` HTML |
| A bare relative reference: `health`, `frobnicate` | `health` — no leading `/` | runtime `404` HTML |
| An opaque `scheme:rest` target: `http:health`, `a:b/health` | `null` | **no status line at all**; connection closed |

The same rule read in the other direction is why three targets that *look* as though they would be refused are not: `///health`, `//127.0.0.1/health` and `http://127.0.0.1:8080//health` all parse to a `/`-prefixed path — three slashes leave nothing for an authority to absorb — so all three reach the router and are answered the contract's JSON `404`, `Content-Type: application/json; charset=utf-8`, measured. A future runtime whose parser classified differently would move a target between these sets, which is precisely why the rule is asserted directly and not just the sample.

One further property of the nine is worth stating on its own, because it is a divergence in *ordering* rather than in body shape: **the four `405`→`404` rows are the only place in this contract where method-before-path does not hold.** §5.4 carries the same caveat so that a reader of either section meets it.

The `URISyntaxException` in two of those bodies is **JDK-emitted and cannot be suppressed through the public API**. Assessed rather than waved through: it discloses the name of a standard-library exception class, which is inferable from the runtime choice alone and is already public in this repository's own documentation and source. It carries no path, no configuration value, no host and no stack frame — the classes of content §12.5 actually forbids. It is recorded here as a known, bounded disclosure rather than treated as clean.
...
| A bare relative reference: `health`, `frobnicate` †‡ | **`400`**, JSON ‡ | application `404`, JSON | **runtime `404`, `text/html`** |
| `http:/health`, `HTTP:/health` ‡ | **`400`**, JSON ‡ | application `404`, JSON | application `404`, JSON |
| `http:health`, `a:b/health` (opaque) ‡ | **`400`**, JSON ‡ | application `404`, JSON | **no response; connection closed** |
| A method token the parser does not recognise (`FROBNICATE`, or lowercase `get`) | application `405` + `Allow` | application `405` + `Allow` | application `405` + `Allow` |
| `CONNECT`, in origin-form or authority-form | application `405` + `Allow` | application `405` + `Allow` | application `405` + `Allow` for origin-form; **runtime `400`, `text/html`** for authority-form † |
| A request offering an upgrade (`Upgrade:` with `Connection: Upgrade`) | application answer for the request underneath the offer | same | same |
...
**‡ Level 1 — the parser validates the request line.** Node's HTTP parser admits origin-form and a complete absolute-form URL with an authority, and decides `400 Bad Request` of its own making — before a request listener runs — for everything else: a target carrying a scheme but no authority (`http:/health`), an opaque `scheme:rest` target, and a bare relative reference with no leading `/` (`health`). This is the mirror image of the Level 3 boundary: where Level 3 hands the application more spellings than the contract defines, Level 1 hands it fewer. The *status* in those cells is therefore the parser's, but the *response* is not: the implementation observes the refusal through the `'clientError'` event and writes the answer itself, so each one carries the contract's headers and a `{"error":"Bad Request"}` body rather than the bare status line Node would emit unattended. Status decided below the application, shape decided by it — which is why those cells read `400`, JSON.

**The *method* is a different case, and it is not a boundary at any tier.** A request whose method the parser does not recognise, and a `CONNECT` request, are both routed by `node:http` to somewhere other than a request listener — the first to `'clientError'`, the second to `'connect'` — and a server that leaves those events unhandled answers the first with a bodyless status line and the second **with nothing at all**. Neither is acceptable under a contract that requires every method other than `GET` and `HEAD` to be answered `405` with `Allow: GET, HEAD`, so an implementation at this tier MUST handle them: a method that is a well-formed RFC 9110 §5.6.2 `token` is answered `405` however unfamiliar its value, and `CONNECT` is answered `405` whatever its target form, because the method is evaluated before the path. The same reasoning applies to an upgrade offer, routed to `'upgrade'` and equally unanswered by default: no tier speaks an upgrade protocol, so all three answer the request underneath the offer and none ever sends `101 Switching Protocols`. The Python and Java tiers reach these answers through their own runtimes without special handling, which is why the rows above are uniform.

**Protocol-level refusals that remain, and the shape they take.** Three conditions are genuinely below routing and are answered by status alone rather than by the contract's vocabulary, because no route was ever determined: a **malformed request line** (`400`) — including a tab where RFC 9112 §3 requires the single space, which Level 1's parser reports under the same error code as an unrecognised method and which an implementation MUST therefore tell apart from one by inspecting the token rather than the code; an **oversized header block** (`431`); and a **request that never completed** within the runtime's own timeout (`408`). These are not contract codes and a consumer MUST NOT expect them from a routed request. What the contract does require is their *shape*, so that a refusal is as machine-readable as a routed answer rather than a bare status line: at **Level 1** each one carries both contract headers and a single-member `{"error":"<reason phrase>"}` body, written by the implementation from the `'clientError'` event; at **Level 2** the same population is shaped by the runtime's own error path through `error_content_type` and `error_message_format`, which supplies the identical JSON body and `Content-Type` but does not add `Cache-Control` — and, for a request line the parser reads as HTTP/0.9 (the table below), sends that body with no status line at all, because the protocol has none. **Level 3** does not participate in this population the same way: its runtime answers the targets its dispatcher refuses with a short `text/html` body (†), and closes the connection outright when its own header-count limit is exceeded. The three tiers therefore agree on *whether* a malformed request is refused and never served, and are documented — not assumed — to differ in how much of the refusal is dressed as JSON.

**Two malformed-request dispositions on which the tiers are measured to differ, recorded rather than harmonized.** Neither is covered by a contract clause, and neither is a defect: each is its own runtime's own reading of a rule the specification leaves to the server.

- **A missing `Host` header.** RFC 9112 §3.2 requires an HTTP/1.1 request to carry exactly one `Host` field and says a server MUST respond with `400` to a request lacking it. **Level 1 does — measured `400`** — and it does so from Node's own host-header check, which runs *before* the application and is not reachable from the `'clientError'` event, so that one refusal is the single response at that tier not shaped by the implementation: it arrives with `Connection: close` and an empty chunked body rather than the contract's JSON. **Levels 2 and 3 are lenient and answer `200`** with the ordinary payload, because neither runtime enforces the field. The asymmetry is accepted for two reasons: the strict answer is the correct one and the tier that gives it is not being asked to stop, while a health endpoint bound to loopback and probed by a container runtime never receives such a request in practice — every probe in §11 and every client in §12 sends `Host`. An implementation MUST NOT *add* leniency at Level 1 to match the others; that would be aligning downward on a rule the specification states with a MUST.
- **Runtime header limits, which protect along different axes per tier.** All three refuse a hostile header block, but they measure different things, so a single expected answer cannot be written for all three. Level 1 limits the **total size** of the block — a single 40 KB field is refused `431` with the contract headers and a `{"error":"Request Header Fields Too Large"}` body, while 200 small fields are served normally. Level 2 limits the **count** — 100 fields is already refused `431`, with the identical JSON body and `Content-Type` but no `Cache-Control` (§12.5), while a single 40 KB field is served. Level 3 answers 100 fields normally and, past its own count limit, **closes the connection with no status line at all**. Level 1 additionally refuses a `NUL` byte in a field value with `400`, which the other two accept. Every one of these is a desirable defence, none is configurable through this contract, and — verified after each — **the listener answers `200` on the very next request in all three cases**, which is the property that matters.
...
| `GET⇥/health HTTP/1.1` (a tab in place of the space) | **`400`**, JSON ‡ | parser accepts it; application sees `/health` and answers `200` | **runtime `400`, `text/html`, "Bad request line"** |
| `·/health HTTP/1.1` (no method at all: the line begins with the space) | **`400`**, JSON ‡ | **`{"error":"Bad Request"}` with no status line** — the parser reads the two remaining words as an HTTP/0.9 request, and RFC 1945 §6 gives an HTTP/0.9 response no status line and no headers | **runtime `405`** — the first word is taken as the method, which is not `GET` or `HEAD` |
...
Error responses carry the same `Content-Type: application/json; charset=utf-8` and `Cache-Control: no-store` headers as the success response. Only the `405` response carries `Allow`. The same single-member shape is reused for the protocol-level refusals of §5.1.2 — `{"error":"Bad Request"}` and `{"error":"Request Header Fields Too Large"}` — even though their status codes are not part of the contract's vocabulary, so that a caller never has to parse two body formats from one endpoint.
...
1. **The method check runs first.** `POST /health` returns `405`, not `404`; `POST /unknown` also returns `405`, because the request never reaches the path comparison. A caller therefore always learns the most actionable fact first: that the method it used is not permitted anywhere on this server. **One measured exception, at Level 3 only:** four request lines whose target its runtime refuses below routing — `OPTIONS *`, `POST *`, `POST //health`, `PUT //health` — are answered `404` by the runtime, because no handler is reached to apply this rule at all. That is the boundary §5.1.2 enumerates and scopes, and it is a property of the dispatcher rather than a choice available to the implementation; for every target that reaches application code, at every tier, the ordering above holds without exception.

    **A corollary of that ordering: a refused method is refused without reading the request body**, and the consequence is measured rather than left to surprise a caller. The method check needs nothing from the body, and §9.3's rule that the endpoint does no work it does not have to applies to a refusal just as much as to a success — so an implementation MUST NOT drain a body it has already decided to reject. Measured with a declared `Content-Length`: a 1 MiB `POST /health` is answered the frozen `405` with `Allow: GET, HEAD` and the JSON body at **all three tiers**, and each listener answers `200` on the next request. At 4 MiB the server has answered and closed before the client finished writing, so **the client's own `send` fails** — `EPIPE` at Levels 1 and 2, `ECONNRESET` at Level 3. That is the correct outcome, not a fault: RFC 9110 §9.3 permits a server to respond before receiving the whole body, and the alternative — reading megabytes solely to discard them — would make an endpoint whose whole purpose is to be cheap into a free amplifier. A client that must read the refusal should send a body small enough to fit the socket buffers (1 MiB does, at every tier) or tolerate the write error and read what the server already sent. **`Expect: 100-continue` does not help, and the measurement is worth recording because the opposite is the natural assumption:** all three runtimes answer `100 Continue` **unconditionally and before routing** — measured for `POST` and for `GET`, including for a method that is about to be refused `405` — so the expectation mechanism cannot be used to discover the refusal in advance. Like the missing-`Host` refusal, that `100` is a response no tier's implementation shapes or can suppress — one more member of the set of runtime-emitted statuses catalogued in §5.1.2, alongside the `400`, `408` and `431` refusals recorded there, and distinct from the `200`, `404` and `405` the contract itself defines. **Nothing here is a size limit on the endpoint**: no tier configures one, and the refusal is of the *method*, at any size.
...
- **The usability rule for a port, stated as far as it is uniform — and no further.** A configured port is usable when it denotes an integer in `1..65535`; anything else falls through to the next link. Measured across fourteen spellings at all three tiers, twelve are treated identically: `8199`, `" 8199"`, `"8199 "` and `08199` are all accepted as `8199` (surrounding whitespace is trimmed, a leading zero is not significant), while `0`, `-1`, `65536`, `0x2007`, `8199.0`, `not-a-number` and the empty string are all refused everywhere. **Two spellings diverge, in opposite directions, and neither is a defect:** `+8199` is accepted as `8199` by Levels 2 and 3 — `int()` and `Integer.parseInt` both admit an explicit plus sign — and refused by Level 1, whose guard is a digits-only test applied before conversion; and `8_199` is accepted as `8199` by **Level 2 alone**, because PEP 515 underscore separators are legal to `int()`, while `Integer.parseInt` raises and Level 1's guard rejects it. Level 1's stricter guard is doing real work rather than being merely pedantic: `Number.parseInt("8_199", 10)` returns **`8`**, so a tier that converted without the digits-only test first would silently bind port 8. **No tier ever binds a port the operator did not write**, every refusal is fail-soft to the next link, and each tier announces the port it actually bound in its start-up line — which is what makes the divergence observable rather than latent. An implementation MUST NOT loosen Level 1's guard to match the others, and MUST NOT tighten a language's own integer contract at the other two; the uniformity this contract requires is of the *served response*, and every one of these spellings either serves the contract on the port written or on the documented default.
...
> Level 3 has no `.env.example` and no runtime-version dotfile. Java has no dotenv convention without a third-party library, and adding one would breach the composition's zero-dependency constraint, so `HealthServer` reads `HEALTH_PORT` and `HEALTH_HOST` through `System.getenv` with `application.properties` supplying the defaults. Both overrides are documented in comments inside that properties file, and the Level 3 README MUST document them as well. The Level 3 runtime pin has no dotfile to live in either; the Level 3 `Dockerfile`'s `eclipse-temurin:25-*-alpine` tags carry it today, and the workflow's `java-version` input MUST carry it as well once that workflow exists.
...
- **Recovery is at wall-clock rate, which is worth stating because "self-cancelling" could be read as "instant".** The drift decays one millisecond per millisecond of idle time, so a drift accumulated by sustained load needs proportionate quiet to clear. Measured under one second of continuous requests on a loopback socket: Level 1 served 13,107 requests and ran +12,106 ms ahead; Level 3 served 7,912 and ran +6,911 ms ahead; Level 2 served 3,780 and ran +2,779 ms ahead. In every case the excess is `requests − elapsed_milliseconds`, which is the bound: **the timestamp can never be further ahead than the number of requests that have been served**, and after 1.2 s of idle each tier's drift had fallen by almost exactly 1,200 ms. Nothing accumulates across quiet periods and no state grows, so this is a saturation artefact rather than a leak — and it is unreachable at any probe rate this endpoint is designed for, since a container health check polls once every 30 seconds (§11.1). Recorded so that a load test is not mistaken for a clock defect.
...
### 9.5 Two serving properties that are implementation guidance, not contract

This contract governs *what* a response contains — its status, its headers, its members and their freshness. It deliberately says nothing about how the bytes are framed on the wire, and **an implementation is not required to produce any particular number of TCP segments, or any particular number of application writes, per response.** TCP is a byte stream: writes may be coalesced or split by either stack independently of what the application did, so a segment count is not a property an implementation can promise or a consumer can rely on. Nothing in this section is normative, and no conformance assertion may be written against it.

What follows is guidance recorded because it is easy to regress and cheap to keep.

- **Prefer emitting the status line, header block and body with one application-level write.** It saves a round trip on a persistent connection and removes any opportunity for a transport to hold part of a response while it waits for an acknowledgement of the rest. It is a latency preference, not a requirement, and a response written in more than one call is still fully conformant.
- **Where a runtime's default is to delay small writes and the handler writes more than once, disable the delay explicitly** rather than depending on the response happening to fit anywhere in particular.

The reason to bother is that a poller times a probe as well as reading it, and a socket made to wait looks exactly like a slow application. On this codebase, a handler that wrote the head and the body separately onto an unbuffered socket, with the transport's small-write delay left enabled, answered keep-alive requests an order of magnitude slower than the work it was doing — a cost produced entirely by framing, and invisible to every assertion about status, headers or body. That is the condition the guidance above avoids.

How each tier happens to emit its response today, recorded so a later edit knows what it is changing:

| Tier | How the response is emitted |
| --- | --- |
| L1 | The socket-level writer concatenates head and body and calls `socket.end` once; the routed path writes through `ServerResponse`, which coalesces already. `node:http` also sets `TCP_NODELAY` on accepted sockets by default |
| L2 | The body is appended to the base class's header buffer and `flush_headers()` writes it in one call — with a fallback to `end_headers()` plus a body write if that buffer is ever not a list, and the HTTP/0.9 path preserved byte-for-byte. `disable_nagle_algorithm = True` on the handler, which `socketserver` applies to every accepted connection |
| L3 | `com.sun.net.httpserver` buffers the head and flushes it with the body when the exchange's stream is closed. `TCP_NODELAY` is left at the module's documented default of off, because the handler does not write twice |

**One genuine requirement does belong here, and it is about the accept queue rather than about framing.** A tier MUST NOT leave its listener's backlog at a value a burst of pollers can overflow: an overflowed queue makes the kernel discard the handshake, and the client then waits out a retransmission timeout — long enough for a probe to call the endpoint down while it is in fact answering every request it receives promptly. Level 1 and Level 3 inherit adequate backlogs from their runtimes; Level 2 sets `request_queue_size` to the platform's `SOMAXCONN` explicitly. That property is asserted per §12.2, because no response-level assertion can reveal it.
...
This section specifies the container definitions the feature requires. **All three tiers now carry one** — a `Dockerfile` and a `.dockerignore` sit at the apex, at Level 2 and at Level 3 — so what follows both prescribes what each image MUST implement and describes files that are in the tree. The composition had no container artifact of any kind before this feature, so every one of those six files was a create rather than an edit.
...
- **A method the runtime does not recognise, and `CONNECT`, both returning `405` with `Allow: GET, HEAD` and the error body — never silence, and never a bare status line.** These MUST be sent over a raw socket, for the same reason as the alias spellings: no client library will transmit `FROBNICATE`, a lowercase `get`, or a `CONNECT` line on request. Both `CONNECT` target forms are asserted, and Level 3's authority-form boundary is the documented exception of §5.1.2 rather than a licence to skip the case. This assertion exists because the condition it covers is invisible to every other one in this list: the runtime routes these requests past the handler entirely, so a tier can satisfy every row of §5.1 while answering them with nothing at all.
- **An upgrade offer answered as the request underneath it** — `200` on the exact path, `404` on any other — and never `101`, so that no tier can be talked into a protocol it does not speak.
- **A malformed request line returning `400`, and an oversized header block returning `431`** where the runtime surfaces either condition to the application (§5.1.2 records which tier surfaces which), each with the shape that section requires. A malformed line MUST be told apart from an unrecognised method by inspecting the method token, not the runtime's error code, because at least one runtime reports both under the same code.
- **The listener still answers `GET /health` with `200` immediately after every one of the refusals above**, asserted on a fresh connection, so a refusal is proven to be a dead end for the request that caused it and never a way to disturb the endpoint.
- **A connection reused for several sequential requests answers every one of them completely**, asserted by reading each response to the end of the body its own `Content-Length` declares and requiring a conformant status line, the contract headers, the full declared body and a distinct `timestamp` per response — so a reused connection cannot serve a truncated answer or replay a cached one. The assertion is about HTTP framing and content; it **MUST NOT** assert a segment count, an application write count or a wall-clock budget, because §9.5 makes none of those a property of this contract.
- **A burst of simultaneous connections is answered within a budget** (§9.5), sized above the burst's healthy cost and below the round-trip timeout a dropped handshake incurs, so an inadequate accept backlog fails the suite instead of degrading a probe in production.
...
Two consequences for whoever writes the teardown, both measured, because each one looks like a server defect and is not:

- **`kill -TERM "$pid"; wait "$pid"` under `set -e` fails at Level 3 and only at Level 3.** `wait` reports the child's status, that status is `143`, and `set -e` aborts the script on it — after a shutdown that was completely orderly. A teardown MUST therefore tolerate `128 + signal`, for example `wait "$pid" || true`, or test the released port rather than the exit status. This is the practical form of the MUST above, and it is the single most likely way for a correct implementation to be reported as broken.
- **A server started as a background job by a non-interactive shell inherits `SIGINT` set to ignore**, so `kill -INT` on it does nothing at any tier. POSIX has the shell set `SIGINT` and `SIGQUIT` to `SIG_IGN` in a background child, `nohup` adds `SIGHUP`, and a runtime that correctly preserves an inherited `SIG_IGN` — as the JVM does — cannot install a handler for it. Verified by reading `SigIgn` in `/proc/<pid>/status`, which shows the two signals masked. `SIGTERM` is unaffected and works on such a process, which is why every teardown in this composition uses `SIGTERM`. An interactive shell, a container stop and a foreground launch all deliver `SIGINT` normally, and the orderly shutdown then runs exactly as specified above.
...
### 12.5 The four diagnostics, and the silence around them

The endpoint's normal operation is silent past its single start-up line, and that silence is a requirement (§13): a resource polled every few seconds would otherwise become the loudest thing in the log, and a log nobody reads is a log that hides the one line that mattered. But silence has a failure mode of its own — a misconfiguration that produces no output is indistinguishable from correctness — so exactly four diagnostics are defined, and no other output is permitted. All four are **bounded**, all four are **sanitized**, and none is per-request.

Three of them fire at start-up and one only on a defect, so a correctly configured process writes **nothing** to `stderr` for its whole lifetime. Where several fire together their order is fixed: the degraded-source line comes first, because a source that never arrived explains everything after it; the rejected-declaration lines follow; and the start-up announcement — the only line on `stdout` — comes last, so a reader meets the caveats before the announcement they qualify. The fourth is the alternative to that announcement rather than a companion to it: a process either binds and announces the address it is serving, or it does not bind and says why. The first two still precede it, because a configuration that degraded is exactly the context in which a bind failure makes sense.
...
**4. The bind-failure line, when the listener cannot start.** A process that could not bind writes **one line to `stderr`** and exits non-zero. The same discipline as above governs its rendering — a configuration-derived host is validated against an allowlist and replaced wholesale with `<unprintable>` if it fails, rather than being escaped piecemeal; an unrecognised failure is reported by stable category rather than by the platform's locale-dependent prose; and all three tiers use the same `<unprintable>` token, so one search spans the composition's logs.

What this item adds is a requirement on the line's *content*, and it exists because of a measured defect. Three unrelated conditions reach a runtime as one indistinguishable error — the port is held by another process (`EADDRINUSE`), the host is an address this machine does not own (`EADDRNOTAVAIL`), and the operating system refused the bind (`EACCES`) — and at Level 3 all three were reported as `is already in use - stop the process holding it, or set HEALTH_PORT to a free port`. An operator who mistyped `HEALTH_HOST` was told to free a port that was never occupied. **A vague diagnostic costs a minute; a confidently wrong one costs an afternoon**, which is why the requirement below is about distinguishing causes and never about guessing between them.

- **Each cause MUST render a distinguishable line, and each line MUST name the override that resolves it.** A port fault points at the port override; a host fault points at the host override. A line that names the wrong override is worse than one that names neither.
- **When the cause cannot be established, the line MUST say so by naming both candidates** rather than selecting one. This is the only permitted vague outcome, and it is strictly preferable to a confident wrong answer.
- **Classification MUST NOT rest on the platform's message alone.** `strerror` text is locale-dependent, so a matched token is decisive but an unmatched one proves nothing. Where a runtime collapses the causes into one error type, a second, locale-independent signal is required — whether any local interface carries the configured address, and whether the port is one the operating system reserves.
- **That second signal MUST NOT be primary, and MUST NOT fire where it is unsound.** Measured on Linux: the whole `127.0.0.0/8` range binds through the loopback route, so `127.0.0.5` binds successfully while `NetworkInterface.getByInetAddress` returns `null` for it. A check that led with the interface lookup would report a genuine occupancy failure on such an address as "not an address of any local interface" — trading one wrong diagnosis for another. Wildcard and loopback addresses are therefore excluded from it.
- **The platform's own message MUST NOT be echoed**, for the same reason a handler failure never renders an exception message: the text is unstable across platforms and locales, so a runbook cannot match on it.

The three tiers' actual output for the same fault — `HEALTH_HOST` (`HOST` at Level 1) set to `192.0.2.99`, an address reserved for documentation by RFC 5737 §3 and owned by no host:

```text
L1  health server cannot start: 192.0.2.99 is not an address of any local interface. Set HOST to an address this host owns, or to 0.0.0.0 for every interface.
L2  server.py: cannot bind 192.0.2.99:8000: address not available on this host
    server.py: HEALTH_HOST must name an address assigned to a local interface. 0.0.0.0 binds every interface and is the default
L3  cannot start: 192.0.2.99 is not an address of any local interface - set HEALTH_HOST to an address this host owns, or 0.0.0.0 for every interface
```

Level 2 uses two lines because that is the shape its entry point already applies to a start-up failure — a statement of the fault followed by the remedy. As with the prefix, the contract constrains the content and the cause vocabulary, not the line count or the prose. The causes each tier distinguishes are the same four: the port is in use, the address is not available here, the bind was refused, and the host does not resolve.
...
Each tier's own README MUST carry the invocation for that tier and MUST refer to this document by its documented location, `docs/health-endpoint.md` in the parent repository (§1.2). Those README sections are a required deliverable of this feature; the apex README is [`../README.md`](../README.md). This document deliberately records no delivery state for them — a normative contract that tracks which README has landed is stale the moment the next one does, so what binds here is the requirement rather than the progress against it. This document remains the normative source for the response contract itself, and any change to the contract is made here first — a README that disagrees with it carries the defect.
