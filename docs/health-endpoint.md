...
### 9.5 The response leaves in one write, and the connection is not made to wait

Two requirements about *how* the response is emitted, rather than what it contains. Both exist because a health endpoint is judged by its latency as well as its body: a poller that times a probe cannot tell the difference between an application that is slow and a socket that was made to wait, and it will report the endpoint down for either.

- **A response MUST be written to the socket in a single write.** Status line, header block and body are one contiguous sequence of bytes, not a head write followed by a body write.
- **A tier MUST NOT let its transport defer that write waiting for an acknowledgement of a previous one.** The first requirement is the substantive one — with a single segment per response there is nothing for Nagle's algorithm to hold — so this second one binds only where a tier writes more than once or where its runtime can delay a lone write. Where it binds and the runtime's default is to delay, the implementation disables the delay explicitly rather than relying on the response happening to fit in one segment.

These are stated as requirements because the cost of missing them was measured, not imagined. With the body written separately from the headers onto an *unbuffered* socket, Nagle's algorithm holds the second small segment until the peer acknowledges the first, and the peer's stack delays that acknowledgement — so on a persistent connection every request after the first completed in **~41 ms** while its headers arrived in ~0.1 ms. The work the endpoint actually performs is around 0.1 ms. The delay was pure framing, it appeared only on keep-alive connections (which is exactly how an orchestrator polls), and no assertion about status, headers or body could see it.

The mechanism is necessarily per-runtime and each tier uses the one its runtime offers:

| Tier | Single write | No deferral |
| --- | --- | --- |
| L1 | The socket-level writer concatenates head and body and calls `socket.end` once; the routed path writes through `ServerResponse`, which coalesces already | `node:http` sets `TCP_NODELAY` on accepted sockets by default |
| L2 | The body is appended to the base class's header buffer and `flush_headers()` writes it in one call — with a documented fallback to `end_headers()` plus a body write if that buffer is ever not a list, and the HTTP/0.9 path preserved byte-for-byte | `disable_nagle_algorithm = True` on the handler, which `socketserver` applies to every accepted connection |
| L3 | `com.sun.net.httpserver` buffers the head and flushes it with the body when the exchange's stream is closed | **Not set, and not needed.** `TCP_NODELAY` is off unless `sun.net.httpserver.nodelay` is set — the module documents the default as `false`, and `ServerConfig` reads it through `Boolean.getBoolean` in a static initializer. Nagle's algorithm has nothing to hold here because there is no second segment to hold: measured at 0.45-0.95 ms per keep-alive request, with the whole response arriving in one read |

**A related requirement concerns the accept queue, and it is recorded here for the same reason: it is a latency property invisible to every response-level assertion.** A tier MUST NOT leave its listener's backlog at a value that a burst of pollers can overflow. Measured against `socketserver`'s default of 5: 40 simultaneous clients produced ~1004 ms *inside `connect()`* for half of them — the kernel discarded the handshake and the client waited out a retransmission timeout — while the exchanges themselves took ~2 ms, and the burst as a whole took 1284 ms against 18 ms once the backlog was raised. A probe with a one-second timeout would have called the endpoint down while it was answering every request it received in single-digit milliseconds. Level 1 (511) and Level 3 (50) inherit adequate backlogs from their runtimes; Level 2 sets `request_queue_size` to the platform's `SOMAXCONN` explicitly.

Both properties are asserted rather than assumed, per §12.2, and each assertion was itself checked against the defective behaviour to confirm it fails when the property is missing — a latency gate that passes both ways protects nothing.
...
- **The whole response arrives in one segment on a persistent connection** (§9.5), asserted by reading a single `recv` on a raw socket for each of several sequential keep-alive requests and requiring the status line, the complete header block *and* the full declared body in every one. This is the deterministic form of the assertion; a wall-clock budget for the same sequence is asserted alongside it to catch a stall arriving by any other route. A tier whose runtime coalesces for it asserts the property all the same, because the assertion is about the bytes on the wire and not about which layer produced them.
